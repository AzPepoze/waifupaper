...
def _is_target_process(proc):
    ...
    is_linux_script = "waifupaper.py" in cmdline
...
def _launch_linux():
    ...
    launcher = os.path.join(linux_pkg_dir, "waifupaper.sh")
    
    if os.path.exists(launcher):
        os.chmod(launcher, 0o755)
        return subprocess.Popen([launcher], cwd=linux_pkg_dir)
    
    print(f"[Dev] Package not found. Using 'waifupaper.py' fallback.")
    linux_src_dir = os.path.join(os.getcwd(), "src", "linux")
    return subprocess.Popen([sys.executable, "waifupaper.py"], cwd=linux_src_dir)

def restart_environment():
    """Orchestrates the full stop-build-start cycle."""
    kill_waifupaper_processes()
    if run_build_script():
        launch_application()
...
if __name__ == "__main__":
    print("--- WaifuPaper Auto-Dev System ---")
